...
def my_DCT(src, n=8, mode='normal'):
    h, w = src.shape
    ...
    if mode == 'normal':
        ...
        for x in range(n):
            for y in range(n):
                for u in range(n):
                    for v in range(n):
                        cosmask[x][y][u][v] = np.cos(((2 * x + 1) * u * np.pi) / (2 * n)) * np.cos(((2 * y + 1) * v * np.pi) / (2 * n))

        for u in range(n):
            for v in range(n):
                cmask[u][v] = getC(u, n) * getC(v, n)

        for row in range(h // n):
            for col in range(w // n):
                frequency_block = src[row * n:row * n + n, col * n:col * n + n]
                for x in range(n):
                    for y in range(n):
                        dst[row * n + x][col * n + y] = np.sum(frequency_block * cmask * cosmask[x][y])
